...
public class TwitterRequest: Comparable, Equatable
{
    ...
    public static func <(lhs: TwitterRequest, rhs: TwitterRequest) -> Bool {
        return lhs < rhs
    }
    ...
    public func fetch(handler: @escaping (_ results: PropertyList?) -> Void) {
    ...
    func performTwitterRequest(request: SLRequest, handler: @escaping (PropertyList?) -> Void) {
        if let account = twitterAccount {
            ...
            request.perform { (jsonResponse, httpResponse, _) in
                ...
                if jsonResponse != nil {
                    do {
                        propertyListResponse = try (JSONSerialization.jsonObject(with: jsonResponse!, options: JSONSerialization.ReadingOptions.mutableLeaves) as TwitterRequest.PropertyList?)!
                    } catch _ {
                        propertyListResponse = nil
                    }
                    
                    /*
                     propertyListResponse = JSONSerialization.JSONObjectWithData(
                     jsonResponse,
            ...
            accountStore.requestAccessToAccounts(with: twitterAccountType, options: nil) { (granted, _) in
                if granted {
                    if let account = accountStore.accounts(with: twitterAccountType)?.last as? ACAccount {
                        ...
                        self.performTwitterRequest(method: request.requestMethod, handler: handler)
                    } else {
    ...
    private func log(whatToLog: AnyObject) {
        print("TwitterRequest: \(whatToLog)")
    }
